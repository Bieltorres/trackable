...
export function CourseModal({ cursoId, isOpen, onClose }: CourseModalProps) {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"stripe" | "pagarme">(
    "stripe"
  );
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center mb-4">
                        <div
                          className={`w-12 h-12 ${color.bg} rounded-full flex items-center justify-center mr-3`}
                        >
                          <User className={`h-6 w-6 ${color.text}`} />
                        </div>
                        <div>
                          <h4 className="font-semibold">
                            {avaliacao.usuario?.name || "Usuário"}
                          </h4>
